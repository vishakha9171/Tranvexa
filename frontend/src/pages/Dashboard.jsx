  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Store latest 5 trips
        setTrips(allTrips.slice(0, 5));

        setMetrics({
          totalVehicles: allVehicles.length,
          activeTrips: allTrips.filter(
            (t) => t.status === 'In Transit'
          ).length,
          deliveredOrders: allTrips.filter(
            (t) => t.status === 'Delivered'
          ).length,
          driversAvailable: allDrivers.filter(
            (d) => d.status === 'Available'
          ).length
        });
      } catch (err) {
        console.error(
          'Error connecting dashboard to logistics pipeline:',
          err
        );
      } finally {
        setLoading(false);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Monitor production line outputs, delivery fulfillment,
            and live asset allocations.
          </p>
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Real-time status tracking logs across regional dispatch fields.
            </p>
          </div>
            ...
            <tbody>
              {trips.length === 0 ? (
                <tr>
                  ...
                  >
                    No active transport operations found in database registry.
                  </td>
                </tr>
              ) : (
                trips.map((trip, index) => (
                  ...
                  >
                    <td className="px-6 py-4 font-medium text-gray-800">
                      {trip.tripId ||
                        `TRIP-${trip._id?.slice(-6) || index}`}
                    </td>

                    <td className="px-6 py-4 text-gray-600">
                      {trip.vehicleId?.vehicleNumber || 'Unassigned'}
                    </td>

                    <td className="px-6 py-4 text-gray-600">
                      {trip.driverId?.fullName || 'Unassigned'}
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold tracking-wide ${getStatusStyle(
                          trip.status
                        )}`}
                      >
                        {trip.status || 'Unknown'}
